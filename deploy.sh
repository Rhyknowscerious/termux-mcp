#!/bin/bash
# termux-mcp2 Deployment Script
# Run this on the Termux device

set -e

echo "=== Termux-MCP2 Deployment ==="
...
# 3. Start termux-mcp2 on port 8000 (ACTIVE)
echo "[3/4] Starting termux-mcp2 on port 8000..."
...
if curl -s http://localhost:8000/ping > /dev/null 2>&1; then
    echo "✅ termux-mcp2 running on http://localhost:8000"
...
echo "Active: termux-mcp2 on port 8000"
echo "Legacy: termux-mcp available on port 666 (not running)"
echo ""
echo "To start legacy (optional): TERMUX_MCP_PORT=666 python -m termux_mcp &"